...
class StatusArray(params: RSParams)(implicit p: Parameters) extends XSModule
  ...
  val replayArray = RegInit(VecInit(Seq.fill(params.numEntries)(RSFeedbackType.feedbackInvalid))) // for perf-eval only
  ...
  (statusArrayValidNext zip statusArrayNext zip replayArrayNext).foreach { case ((statusValid, status), replay) => when(statusValid === 0.B) { replay := RSFeedbackType.feedbackInvalid } }
  io.rsFeedback := VecInit((0 until 5).map(index => statusArrayValid.zip(statusArray).zip(replayArray).map {
    case ((statusValid, status), replay) => statusValid && replay === index.U
  }.reduce(_ || _)))
